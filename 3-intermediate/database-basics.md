...
For the rest of the article we'll also be using the word "database" to refer to database systems, so just keep that in mind!
...
It's the interface we use to actually create, read, update, and delete data from databases. 

Without it, databases aren't actually that practical. As mentioned before, databases on their own lack a lot of the efficiency, security, and constraints that  A lot of manual labour can be required to work with the data, and they lack many important   



Basically it's an interface the 

On their own, databases aren't actually that pratical. If you're using a spreadsheet, you'll have to do a lot of manual work to The DBMS is a type of software that allows users to manage a database. the 
